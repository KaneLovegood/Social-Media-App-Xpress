...
function getCallLogViewModel(
  callLog: NonNullable<ChatMessage["callLog"]>,
  currentUserId: string,
): CallLogViewModel {
...
export default function MessageBubbleCard({
  ...
  const replyPreviewSenderName =
    ...
    resolveSenderName(message.replyPreview?.senderId);
    ...
    return (
      <div className="w-58 overflow-hidden rounded-2xl border border-[#b8c5db] bg-[#dfe9f6] px-4 py-3 text-[#223457] shadow-sm">
        ...
        <div className="mt-1 flex items-center gap-2 text-base font-medium text-[#6a7b98]">
          {callView ? (
            <span
              className={`relative flex h-6 w-6 items-center justify-center rounded-full border ${callView.ringClassName ?? "border-zinc-200 bg-white"}`}
            >
              <callView.Icon
                className={`relative h-4 w-4 ${callView.iconClassName}`}
              />
            </span>
          ) : null}
  ...
  return (
    ...
    >
      ...
      {message.isRecalled ? (
        <p className={`italic ${isOwn ? "text-orange-100" : "text-zinc-500"}`}>
        ...
        <div className="flex flex-col gap-2">
          {message.messageType === "IMAGE" && message.fileUrl && (
            <img
              ...
              onClick={() => {
                if (onImageClick) {
                  const imageSenderName = senderName;
                  const timestamp = new Date(message.createdAt).toLocaleString(
                    "vi-VN",
                  );
                  onImageClick(message.fileUrl!, imageSenderName, timestamp);
                }
              }}
            />
          )}
          ...
          {message.messageType === "FILE" && message.fileUrl && (
            ...
            >
              <div
                className={`p-2 rounded-full shrink-0 ${isOwn ? "bg-white/30 text-white" : "bg-white text-gray-700"}`}
              >
                <FileText size={20} />
              </div>
              ...
              <Download
                size={16}
                className={`ml-2 ${isOwn ? "text-white/80" : "text-gray-500"}`}
              />
            </a>
          )}
          ...
          {message.content && (
            <div
              className={`wrap-break-word ${message.senderId === "AI_ASSISTANT" || message.senderId === "SYSTEM" ? "prose prose-sm prose-slate max-w-none [&>*:first-child]:mt-0 [&>*:last-child]:mb-0" : "whitespace-pre-wrap"}`}
            >
